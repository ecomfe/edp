 ...
 */
function BaiduCloudStorage(ak, sk, maxSize) {
    this.ak = ak;
    this.sk = sk;
    ...
    if (opt_prefix) {
        if (stat.isFile()) {
            ...
            if (ext && ext == path.extname(opt_prefix)) {
                ...
                objectName = '/' + opt_prefix + '/' + path.basename(localFile);
            }
        }
    } else {
 ...
 * @param {string=} opt_prefix The target prefix path.
 */
BaiduCloudStorage.prototype.upload = function(bucketName, localFile, opt_prefix) {
    var url = require('url');
    ...
    if (stat.isDirectory()) {
        ...
        fs.readdirSync(localFile).forEach(function(item){
            if (item.indexOf('.') === 0 || item == 'CVS') {
                ...
                me.upload(bucketName, path.join(localFile, item), path.join(opt_prefix, item));
            }
        });
        return;
    } else if (stat.isFile()) {
        if (stat.size > this.maxSize) {
            console.error("%s size = [%s], maxSize = [%s], ignore it.", localFile,
    ...
    var req = http.request(options, function(res) {
        if (res.statusCode === 200) {
            ...
            if (cdnHost) {
                bcsUrl = bcsUrl.replace(/^http:\/\/(s3\.bae|bs)\.baidu\.com/, cdnHost);
            }
            console.log(bcsUrl);
        } else {
