...
function readDir( dir, startDir, conf, result ) {
    ...
    // 读取当前目录下的'.htmlhintrc'配置文件
    var htmlhintRcFile = dir + '/.htmlhintrc';
    if ( fs.existsSync( htmlhintRcFile ) ) {
        var rcBuffer = fs.readFileSync( htmlhintRcFile );
...
function detectHTML( file, startDir, conf, result ) {
    conf = conf || require( 'edp-htmlhint' ).config;

    var data = {};
    result.push( data );
    data.file = path.relative( startDir, file );

    var htmlhint = require( 'edp-htmlhint' );

    var source = fs.readFileSync( file );

    var errors = htmlhint.lint( source.toString( 'UTF-8' ), conf );
