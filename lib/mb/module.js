    ...
    estraverse.replace(root, {
        enter: function (element) {
            if (element.type == SYNTAX.CallExpression 
                ...
                var deps = [SYNTAX_REQUIRE, SYNTAX_EXPORTS, SYNTAX_MODULE].concat(info.deps);
...
function createLink(source, target) {
    return "define('" + source + "', ['" + target + "'], function (main) { return main; });"
}
...
function combine(moduleId, options) {
    ...
    while (needLoad.length > 0) {
        ...
        // 不通过file去匹配moduleId
        //res = parseFile(util.getModuleId(file, options['module-config']), fs.readFileSync(file, 'utf-8'));
