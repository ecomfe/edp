...
function readDir( dir, startDir, conf, result ) {
    ...
    if ( fs.existsSync( csslintRcFile ) ) {
        ...
        try {
            ...
            console.error( '请使用 JSON 格式的 .csslintrc 配置' );
            process.exit( 1 );
        }
    }
 ...
 * @param {string} status 文字表示的状态
 * @return {string}
 */
function getFormatText( text, status ) {
    if ( colors.mode === 'none' ) {
        return text;
    }
...
function outputMessage( message ) {
    ...
    if ( message.line && message.col ) {
        position = getFormatText( '[L' + message.line + ',C' + message.col + '] ', 'info' );
    }
    ...
    console.log( position + getFormatText( failInfo, message.type ) );
...
function outputTitle( title ) {
    console.log();
    console.log( getFormatText( title, 'title' ) );
    ...
    if (target) {
        ...
        if ( !fs.existsSync( target ) ) {
            ...
            if ( fsStat.isDirectory() ) {
                ...
                detectCSS( target, require( 'path' ).dirname( target ), null, result );
            }
        }
    } else {
